...
gridStr = [row1,row2,row3,row4,row5,row6,row7,row8,row9,row10,row11,row12,row13,row14,row15,row16,row17,row18,row19,row20]
grid = gridStr.map { |string| string.split(" ").map {|s| s.to_i }  }
...
def largestProduct(length, grid)
  horizontalPoints = Array.new
  verticalPoints = Array.new
  leftDiagPoints = Array.new
  rightDiagPoints = Array.new
  
  for i in 0...length
    horizontalPoints << Point.new(0, i)
    verticalPoints << Point.new(i, 0)
    leftDiagPoints << Point.new(length - i - 1, i)
    rightDiagPoints << Point.new(i, length - i - 1)
  end

  greatestProduct = 0
  for x in 0...grid.count - 4
    for y in 0...grid.count - 4
      horizProd = productOfPoints(horizontalPoints, x, y, grid)
      verticalProd = productOfPoints(verticalPoints, x, y, grid)
      leftDiagProd = productOfPoints(leftDiagPoints, x, y, grid)
      rightDiagProd = productOfPoints(rightDiagPoints, x, y, grid)
      [horizProd, verticalProd, leftDiagProd, rightDiagProd].each do |product|
...
def productOfPoints(points, x, y, grid)
  currentPoints = points.map { |point| Point.new(point.x + x, point.y + y) }.map { |point| grid[point.x][point.y] }
  currentPoints.inject(:*)
end

puts largestProduct(4, grid)
