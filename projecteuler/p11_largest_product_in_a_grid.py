...
def checkright(row, index):
    ...
    if index >= 17:
        ...
        for n in range(index, index + 4):
            product = product * row[n]
        return product

def checkdown(row1,row2,row3,row4,index):
    return row1[index]*row2[index]*row3[index]*row4[index]

def checkdiagright(row1,row2,row3,row4,index):
    return row1[index]*row2[index+1]*row3[index+2]*row4[index+3]

def checkdiagleft(row4,row3,row2,row1,index):
    return row4[index]*row3[index+1]*row2[index+2]*row1[index+3]

for x in range(0,20):
    for y in range(0,20):
        if checkright(grid[x],y) > largest_product:
            largest_product = checkright(grid[x],y)

for x in range(0,17):
    for y in range(0,20):
        if checkdown(grid[x],grid[x+1],grid[x+2],grid[x+3],y) > largest_product:
            largest_product = checkdown(grid[x],grid[x+1],grid[x+2],grid[x+3],y)

for x in range(0,17):
    for y in range(0,17):
        if checkdiagright(grid[x],grid[x+1],grid[x+2],grid[x+3],y) > largest_product:
            largest_product = checkdiagright(grid[x],grid[x+1],grid[x+2],grid[x+3],y)
